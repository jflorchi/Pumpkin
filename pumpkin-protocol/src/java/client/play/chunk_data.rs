...
use pumpkin_data::biome_remap::remap_biome_id_for_version;
...

        {
            ...
            for (block_palette, biome_palette) in block_sections.iter().zip(biome_sections.iter()) {
                ...
                if version < &CURRENT_MC_VERSION {
                    block_network.remap_ids(|id| remap_block_state_for_version(id, *version));
                }
                ...
                if version < &CURRENT_MC_VERSION {
                    biome_network.remap_ids(|id| remap_biome_id_for_version(id, *version));
                }
